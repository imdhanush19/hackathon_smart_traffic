...
class Car(pygame.sprite.Sprite) :
    choice=0
    def __init__(self,spawn_direction,turn_dir) :
        super().__init__()
    ...
    def update(self) :
        ...
        if self.s_dir == 0 :
            ...
            if not(self.notstopped and not signal0.state and self.rect.colliderect(centerrect)) :  
                projRect = self.image.get_rect(center = (self.rect.centerx,self.rect.centery + 5))
        ...
        if self.s_dir == 1 :
            ...
            if not(self.notstopped and not signal1.state and self.rect.colliderect(centerrect)) : 
                projRect = self.image.get_rect(center = (self.rect.centerx-5,self.rect.centery))
                for car in car_group :
                    if projRect.colliderect(car) :
        ...
        if self.s_dir == 2 :
            ...
            if not(self.notstopped and not signal2.state and self.rect.colliderect(centerrect)) :
                projRect = self.image.get_rect(center = (self.rect.centerx,self.rect.centery - 5))
                for car in car_group :
                    if projRect.colliderect(car) :
        ...
        if self.s_dir == 3 :
            ...
            if not(self.notstopped and not signal3.state and self.rect.colliderect(centerrect)) : 
                projRect = self.image.get_rect(center = (self.rect.centerx+5,self.rect.centery))
                for car in car_group :
                    if projRect.colliderect(car) :
...
centerrect = pygame.Rect(WIDTH/2 - 30, HEIGHT/2 - 30, 60,60)
...
def check_signal() :
    ...
    for car in car_group :
        if car.s_dir == green_signal and car.notstopped :
            there_is_a_car = True
    if there_is_a_car == False :
        signal_list[green_signal].state = 0
        green_signal = get_car_number()
        signal_list[green_signal].state = 1
...
while run :
    ...
    pygame.draw.line(screen, (0,0,255), (0,HEIGHT/2 - displacement), (WIDTH,HEIGHT/2 - displacement), 2)
    #centerrect
    pygame.draw.rect(screen,(255,0,255), (WIDTH/2 - 30, HEIGHT/2 - 30, 60, 60),2)
